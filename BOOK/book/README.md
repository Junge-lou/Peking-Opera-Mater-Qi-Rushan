...
* 将上述代码保存为 `zhvt-classic.cls`保存在主文档的相同目录。
* 下载 [gezhu.sty](https://link.zhihu.com/?target=https%3A//raw.githubusercontent.com/Salttz/gezhu/master/gezhu.sty) 文件，也保存在著文档相同目录，需要割注的段落要位于 `withgezhu`环境中，割注以 `\gezhu{...}`调用。实际使用中，发现割注环境不能跨章节。
* 以 `xelatex`编译，输出 PDF。
